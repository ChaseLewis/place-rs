...
export class ConfigTableHandle {
  ...
  /**
   * Access to the `config_id` unique index on the table `config`,
   * which allows point queries on the field of the same name
   * via the [`ConfigConfigIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.config.config_id().find(...)`.
   *
   * Get a handle on the `config_id` unique index on the table `config`.
   */
  config_id = {
    ...
    find: (col_val: number): Config | undefined => {
      for (let row of this.tableCache.iter()) {
        if (deepEqual(row.config_id, col_val)) {
          return row;
        }
      }
    },
