...
/**
 * Table handle for the table `disconnected_players`.
 *
 * Obtain a handle from the [`disconnectedPlayers`] property on [`RemoteTables`],
 * like `ctx.db.disconnectedPlayers`.
 *
 * Users are encouraged not to explicitly reference this type,
 * but to directly chain method calls,
 * like `ctx.db.disconnectedPlayers.on_insert(...)`.
 */
export class DisconnectedPlayersTableHandle {
  ...
  /**
   * Access to the `identity` unique index on the table `disconnected_players`,
   * which allows point queries on the field of the same name
   * via the [`DisconnectedPlayersIdentityUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.disconnectedPlayers.identity().find(...)`.
   *
   * Get a handle on the `identity` unique index on the table `disconnected_players`.
   */
  identity = {
