...
export const TileBar = (props: { hide: boolean, downloadImage?: () => void }) => {
    ...
    // Every time a bar is moved, ensure the reset layout button is visible
    useEffect(() => {
        if (!placeStore.showResetLayoutButton && 
            (placeStore.tileBarPosition.isPinned || placeStore.favoriteBarPosition.isPinned)) {
            placeStore.setShowResetLayoutButton(true);
        }
    }, [placeStore.tileBarPosition.isPinned, placeStore.favoriteBarPosition.isPinned]);

    const titleBarClass = useMemo(() => {
        let className = "tile-bar-content";
    ...
    if(placeStore.isMobile) {
        return (
            <Tooltip title="Download Image">
                <Flex gap="4px" className="tile-bar mobile"
                    align="center"
                    justify="center"
                    onClick={props.downloadImage}
                >   
                    <CloudDownloadOutlined style={{ fontSize: "25px", padding: "5px 0px" }} />
                </Flex>
            </Tooltip>
        );  
    }
    ...
    return (
        <DraggableContainer
            position={placeStore.tileBarPosition}
            onPositionChange={placeStore.updateTileBarPosition}
            className="tile-bar"
        >
